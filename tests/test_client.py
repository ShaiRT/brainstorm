...
_IP, _PORT = '127.0.0.1', 4000
_SERVER_ADDRESS = _IP, _PORT
_SERVER_ADDRESS_STR = f'{_IP}:{_PORT}'
_SERVER_BACKLOG = 1000
_CLIENT_PATH = pathlib.Path(__file__).absolute().parent.parent / 'brainstorm'
...
def test_connection(get_message):
    bs.upload_thought(_SERVER_ADDRESS_STR, _USER_1, _THOUGHT_1)
    message = get_message()
    assert message
...
def test_user_id(get_message):
    ...
    bs.upload_thought(_SERVER_ADDRESS_STR, _USER_2, _THOUGHT_1)
    user_id, timestamp, thought = get_message()
...
def test_thought(get_message):
    ...
    bs.upload_thought(_SERVER_ADDRESS_STR, _USER_1, _THOUGHT_2)
    user_id, timestamp, thought = get_message()
...
def test_timestamp(get_message):
    ...
    bs.upload_thought(_SERVER_ADDRESS_STR, _USER_2, _THOUGHT_2)
    user_id, timestamp, thought = get_message()
    _assert_now(timestamp)
...
def test_cli(get_message):
    ...
    process = subprocess.Popen(
        ['python', '-m', _CLIENT_PATH, 'upload-thought', f'{host}:{port}', str(_USER_1), _THOUGHT_1],
