...
class Database:
    ...
    def save_snapshot(self, snapshot):
        query = {'user_id': snapshot['user_id'], 'datetime': snapshot['datetime']}
        update = {'$set': snapshot}
    ...
    def get_snapshot(self, user_id, snapshot_id):
        ...
        ret['available_results'] -= {'snapshot_id', 'datetime'}
        ret['available_results'] = list(ret['available_results'])
        if not ret['available_results']:
            ret['available_results'] = None
        return ret

    def get_snapshot_result(self, user_id, snapshot_id, result):
        query = {'user_id': user_id, 'snapshot_id': snapshot_id}
        projection = {'_id': False, result: True}
