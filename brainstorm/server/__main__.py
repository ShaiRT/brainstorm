...
@click.option('host', '-h', '--host', default='127.0.0.1', show_default=True, help='the server host')
@click.option('port', '-p', '--port', default=8000, show_default=True, help='the server port')
@click.option('path', '--path', default='data', show_default=True, help='directory for blob storage')
...
def run_server(url, *, host='127.0.0.1', port=8000, path='data', tb=False):
    '''run the server to recieve snapshots and post to a message queue in given url.
    '''
    try:
