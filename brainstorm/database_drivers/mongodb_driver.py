...
class MongodbDriver:
    ...
    def save_snapshot(self, snapshot):
        ...
        if result.upserted_id is None:
            return
        ret_doc = pymongo.ReturnDocument.AFTER
    ...
    def get_snapshot(self, user_id, snapshot_id):
        ...
        Returns:
            dict: snapshot
            (with 'snapshot_id', 'datetime' and 'available_results')
        """
        ...
        ret['available_results'] = [key for key, val in snapshot.items()
                                    ...
                                    {'snapshot_id', 'datetime', 'user_id'}]
        if not ret['available_results']:
            ret['available_results'] = None
        return ret

    def get_result(self, user_id, snapshot_id, result_name):
        """Get the requested result of the
        snapshot with snapshot_id of user with user_id
